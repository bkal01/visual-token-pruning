...
def run():
    ...
    return sample, vision_inference_context.surviving_visual_indices, result.image_grid_thw.cpu(), spatial_merge_size, patch_size
...
def main():
    ...
    for step_idx in range(1, len(surviving_visual_indices)):
        ...
        plt.savefig(f"assets/vision_attention/{id}/step_{step_idx}.png", bbox_inches='tight', pad_inches=0)
        plt.close()